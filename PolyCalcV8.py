...
attackers = 'warrior, warrior, Mind bender'  # input("What are the attacking units?(Seperate list with ', '; capitalisation does not matter)\n")
...
def starValue(attackDamage, defenderHealth, attacker, retaliationDamage, attackerHealth, defenderCost, attackerCost, multiplier):
    attackValue = 1 + (int(attackDamage) / int(defenderHealth))
    retaliationCost = 1 + (int(retaliationDamage) / int(attackerHealth))

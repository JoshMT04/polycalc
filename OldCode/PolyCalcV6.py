...
# If the same unit is inputted the calc can not work out which is which and it fails
...
defender = input("What is the defending unit? \n")
...
def PolyDamageCalc(attacker, attacker_health, defender, defender_health):
    ...
    if attackers_df_new.at[attacker, 'safe_dist'] == True or units_df.at[defender, 'Retaliate'] == False:
        defenseResult = 0
...
def PolyAttackScore(attacker, attacker_health, defender, defender_health):
    attackForce = float(units_df.loc[attacker, 'Attack']) * (float(attacker_health) / attackers_df_new.loc[attacker, 'unit_in_unit_health'])
    defenseForce = float(units_df.loc[defender, 'Defense']) * (float(defender_health) / float(units_df.loc[defender, 'Health'])) * defenseBonus
    totalDamage = attackForce + defenseForce
    attackResult = (attackForce / totalDamage) * float(units_df.loc[attacker, 'Attack']) * 4.5
    defenseResult = (defenseForce / totalDamage) * float(units_df.loc[defender, 'Defense']) * 4.5

    safe_dist_value = str(attackers_df_new.at[attacker, 'safe_dist'])
    retaliate_bool = bool(units_df.at[defender, 'Retaliate'])
    
    if safe_dist_value == 'T' or retaliate_bool == False:
        defenseResult = 0
